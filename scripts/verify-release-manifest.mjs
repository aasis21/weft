...
import { validateInstallDescriptor } from "./release-layout.mjs";
...
export function verifyReleaseManifest(directory) {
  ...
  const install = validateInstallDescriptor(manifest.install);
  ...
  for (const name of [...install.bundles, install.skill]) {
    if (!REQUIRED_RELEASE_FILES.includes(name)) {
      throw new Error(`Release install metadata references an unverified payload: ${name}`);
    }
  }
